...
required_packages <- c("quantmod", "ggplot2", "dplyr", "lubridate", "zoo")
...
We perform EDA on the dataset. We first check the structure of the data and the 

```{r}
# EDA: Check data types and summary
str(aapl_data)  # Structure of the data
summary(aapl_data)  # Summary statistics
```

We see that the data contains 7 variables. This is summarised below.
...
Before going further, we check for missingness in the data.

```{r}
# Check for missing values
missing_values <- sapply(aapl_data, function(x) sum(is.na(x)))
print(missing_values)
```
We see that there is no missing data, which is expected since this dataset is sourced from Yahoo Finance, a platform that aggregates stock information from publicly available sources.

We can make several plots to visualise the data. We begin by viewing plots of the closing price and trading volume over time.

```{r}
# Plot: Closing Price Over Time
p1 <- ggplot(aapl_data, aes(x = Date, y = Close)) +
  geom_line(color = "blue") +
  labs(title = "AAPL Closing Price Over Time", x = "Date", y = "Closing Price") +
  theme_minimal()

# Plot: Volume Over Time
p2 <- ggplot(aapl_data, aes(x = Date, y = Volume)) +
...
aapl_data <- aapl_data %>%
  ...
  na.omit()  # Remove rows with NA values for daily returns

# Plot: Daily Returns
p3 <- ggplot(aapl_data, aes(x = Date, y = Daily_Return)) +
  geom_line(color = "green") +
  labs(title = "AAPL Daily Returns", x = "Date", y = "Daily Return") +
  theme_minimal()

# Calculate the 30-day moving average and remove NA values
aapl_data <- aapl_data %>%
  mutate(MA30 = rollmean(Close, k = 30, fill = NA, align = "right")) %>%
  na.omit()  # Remove rows with NA values for moving average

# Plot: Moving Average (30 days)
p4 <- ggplot(aapl_data, aes(x = Date)) +
